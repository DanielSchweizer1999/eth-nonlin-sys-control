...
for all
\begin{itemize}
    \item $x, y \in B (x_0, r) = \{x \in \mathbb{R}^n | \|x- x_0\| \leq r\}$
    \item $t \in [t_0, t_1]$
\end{itemize}
Then there exists some $\delta > 0$ such that the state equation
\noindent\begin{equation*}
...
The solution exists within the domain on which $f$ is Lipschitz for \textit{some time} (potentially very short). However, the solution may e.g.\ have finite escape time and leave the Lipschitz domain of $f$, potentially leading to non-existence after some time.

% TODO: Uncomment it if you're interested in the proof
...
\begin{enumerate}
    \item The Lipschitz condition is stronger than continuity. Namely, some continuous functions are not Lipschitz, e.g.,
          \noindent\begin{equation*}
              f (x) = x^{1/2}
          \end{equation*}
    ...
    \item Lipschitz is only a \textbf{sufficient} condition!
\end{enumerate}
...
\textbf{Note} that the solution exists only in $w$ for all time, not in the entire domain. Key concept: the solution must \textit{stay} within the domain on which $f$ is Lipschitz.

\subsection{Continuity and Differentiability}

The solution of $\dot{x} = f(t, x)$ should continuously depend on $t_0$, $x_0$, $f(x,t)$.

\subsubsection{Dependence on Initial Time}
...
\subsubsection{Dependence on Parameters}
...
We can always find a $\delta$ so that
\begin{itemize}
    \item if the initial condition $z_0$
    \item and the parameter $\lambda$
\end{itemize}
are close enough to the nominal values $y_0, \lambda_0$ then the solution to stays $z$ within a tube around $y$ for some time.
...
The solution $S(t)$ tells us at which points in time $x(t,\lambda)$ will be how sensitive to changes in the parameters.

\textbf{Note} that $A (t, \lambda_0)$ is evaluated at $x(t, \lambda_0)$ and not $x(t, \lambda)$ (same for $B$).

\newpar{}
\ptitle{Numerical Solution Approach}

One can
\begin{itemize}
    \item either integrate the sensitivity equation jointly in $\dot{x}$ and all entries of $\dot{S}$.
    \item or integrate $\dot{x}$ first and then plug $x(t, \lambda_0)$ into the sensitivity equation and integrate $\dot{S}$.
\end{itemize}
