...
\begin{align*}
    y^{(2)} & = \frac{\partial (L_f h(x))}{\partial x}[f(x) + g(x)u] = L_f^2 h(x) + L_g L_f h(x) u                 \\
    y^{(N)} & = \frac{\partial (L_f^{(N-1)} h(x))}{\partial x}[f(x) + g(x)u] = L_f^N h(x) + L_g L_f^{(N-1)} h(x) u \\
\end{align*}
...
\begin{enumerate}
    \item The matrix $\mathcal{M}(x) = [g(x), ad_fg(x), \dots , ad_f^{(n-1)}g(x)]$ has full column rank ($RANK(\mathcal{M}(x)) = n$)
    \item The distribution $\Delta = SPAN\{g, ad_fg, \dots, ad_f^{(n-2)}g \}$ is involutive
\end{enumerate}
...
\begin{align*}
    ...
    \delta(z)  & = \gamma(x) [(\hat{\alpha}(x)-\alpha(x)) + (\hat{\beta}(x) - \beta(x) ) KT - \hat{\beta}(x)K(\hat{T}_2(x) - T(x))]
\end{align*}
